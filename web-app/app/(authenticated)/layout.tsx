...
import { LayoutDashboard, ArrowLeftRight, Tag, Users, LogOut, Bell } from "lucide-react"
  ...
  return (
    <aside className="fixed left-0 top-0 flex flex-col w-[220px] h-screen bg-surface border-r border-border z-40">
      ...
      <div className="flex items-center gap-2 h-16 px-5 border-b border-border">
        <Image src="/images/logo.png" alt="Keeperme" width={28} height={28} />
...
function TopBar() {
  const pathname = usePathname()
  const title = pageTitles[pathname] || (pathname.startsWith("/groups/") ? "Group Details" : "")
  const initials = `${mockUser.name[0]}${mockUser.lastName[0]}`
